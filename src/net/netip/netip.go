...
func (ip Addr) As16() (a16 [16]byte) {
	binary.BEPutUint64(a16[:8], ip.addr.hi)
	binary.BEPutUint64(a16[8:], ip.addr.lo)
	return a16
}
...
func (ip Addr) As4() (a4 [4]byte) {
	if ip.z == z4 || ip.Is4In6() {
		binary.BEPutUint32(a4[:], uint32(ip.addr.lo))
		return a4
	}
	if ip.z == z0 {
...
func (ip Addr) AsSlice() []byte {
	...
	case z4:
		var ret [4]byte
		binary.BEPutUint32(ret[:], uint32(ip.addr.lo))
		return ret[:]
	default:
		var ret [16]byte
		binary.BEPutUint64(ret[:8], ip.addr.hi)
		binary.BEPutUint64(ret[8:], ip.addr.lo)
		return ret[:]
	}
}
...
func (ip Addr) marshalBinaryWithTrailingBytes(trailingBytes int) []byte {
	...
	case z4:
		...
		binary.BEPutUint32(b, uint32(ip.addr.lo))
	default:
		z := ip.Zone()
		b = make([]byte, 16+len(z)+trailingBytes)
		binary.BEPutUint64(b[:8], ip.addr.hi)
		binary.BEPutUint64(b[8:], ip.addr.lo)
		copy(b[16:], z)
	}
	return b
}
...
func (p AddrPort) MarshalBinary() ([]byte, error) {
	...
	binary.LEPutUint16(b[len(b)-2:], p.Port())
	return b, nil
}
...
func (p *AddrPort) UnmarshalBinary(b []byte) error {
	...
	*p = AddrPortFrom(addr, binary.LEUint16(b[len(b)-2:]))
	return nil
}
